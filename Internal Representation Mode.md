# Internal Representation Mode for StyleGAN3

Rendering out videos using the "Internal Representation" mode (only available for StyleGAN3) is really interesting because you get to see the inner layers of the model, which for me partially reveals what's going on within the model. This is particularly powerful if you render multiple videos using the same seed and then composite together tastfully in After Effects. For example, I used this technique in the [Internal Rep](https://www.jasonfletcher.info/vjloops/internal-rep.html) and [Fashion Pose](https://www.jasonfletcher.info/vjloops/fashion-pose.html) VJ packs.
...
ALL USING THE SAME SEED, BUT WITH DIFFERENT LAYERS AND CHANNELS
...
Render Video - Internal Representation:  
ALL USING UNIQUE SEEDS, LAYERS, CHANNELS
