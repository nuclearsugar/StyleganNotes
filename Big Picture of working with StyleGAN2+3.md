...
5. Transfer learn the model using your dataset for between 8 to 48 hours (or much longer if you're able). Doesn't have to be continous since training can be resumed using the last saved PKL checkpoint. Here are some [scripts](https://github.com/nuclearsugar/StyleganNotes/blob/main/Shortcuts%20for%20StyleGAN.md) and [training tips](https://github.com/nuclearsugar/StyleganNotes/blob/main/Training%20StyleGAN%202%2B3.md).
...
### Tips
- After much experimenting, I'd found that the StyleGAN3 codebase is more mature than the StyleGAN2 codebase. And even better is the [StyleGAN3-fun](https://github.com/PDillis/stylegan3-fun) codebase, which luckily still supports the StyleGAN2 approach. Personally I prefer the StyleGAN2 approach to how it morphs visually, it trains about x2 faster, and the generated video seems to be smoother.
- One thing that kept tripping me up is that the image dataset resolution (512x512 is ideal currently) must match the PKL pre-trained model resolume (FFHQ-512x512.pkl). Otherwise it will throw a cryptic error when trying to train.
